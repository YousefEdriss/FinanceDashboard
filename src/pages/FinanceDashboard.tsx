...
import { Plus, Pencil, Trash2, X, Check, ChevronDown, ChevronUp, DollarSign, RefreshCw, Zap, Edit2, Download, Upload } from 'lucide-react';
    ...
    return (
      <span className="inline-flex items-center gap-1.5">
        <input autoFocus type="text" inputMode="decimal" value={draft}
          ...
          className="w-28 text-sm outline-none rounded-lg px-2 py-0.5 font-mono"
...
function StatCard({ label, icon, color, delay = 0, children, action }: {
  ...
  delay?: number; children: React.ReactNode; action?: React.ReactNode;
}) {
  return (
    <div className="card card-corners p-4 md:p-5" style={{ animationDelay: `${delay}ms` }}>
      <div className="flex items-start justify-between mb-3">
        <span className="text-xs font-mono font-medium tracking-widest uppercase"
          style={{ color: 'rgba(168,85,247,0.7)' }}>{label}</span>
...
function RateBadge({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <span className="inline-flex items-center gap-2 text-xs px-3 py-1.5 rounded-full font-mono"
      ...
      <span style={{ color: `${color}90` }}>{label}:</span>
      <strong>{value}</strong>
    </span>
  );
}
...
export default function FinanceDashboard() {
  ...
  const data: FinanceData = { ...DEFAULT_FINANCE, ...rawData, spendingEntries: rawData.spendingEntries ?? [] };
  ...
  function addFreelancing() {
    ...
    const entry: FreelancingEntry = {
      id: genId(), amount,
      date: new Date().toLocaleDateString('en-GB'), note: flNote || '',
    };
  ...
  function addSpending() {
    ...
    if (!spAmt.trim() || isNaN(amount) || amount <= 0) {
      setSpError('Please enter a valid amount.');
      return;
    }
    setSpError('');
    const entry: SpendingEntry = {
      id: genId(), amount, category: spCat, date: spDate, note: spNote || undefined,
    };
    setData((d) => ({
      ...d,
      spendingEntries: [...(d.spendingEntries ?? []), entry],
      cashEGP: d.cashEGP - amount,
    }));
    ...
    if (editSubId) {
      setData((d) => ({
        ...d, subscriptions: d.subscriptions.map((s) =>
          s.id === editSubId ? { ...s, name: subName, cost, currency: subCurrency } : s),
      }));
      setEditSubId(null);
    } else {
  ...
  function exportData() {
    ...
    a.href = url;
    a.download = `mydash-${todayISO()}.json`;
    a.click();
  ...
  function importData(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (evt) => {
      try {
        ...
        alert('Invalid data file — make sure you exported from MyDash.');
      }
    };
  ...
  return (
    <div className="p-4 md:p-6 min-h-full">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6"
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => importRef.current?.click()}
            className="btn-ghost flex items-center gap-2 text-sm px-3 py-2" title="Import data from another device">
            <Upload size={13} /> <span>Import</span>
          </button>
          <button onClick={exportData}
            className="btn-ghost flex items-center gap-2 text-sm px-3 py-2" title="Export data to transfer to another device">
        ...
        <div className="card lg:col-span-2 p-4 md:p-5" style={{ animationDelay: '200ms' }}>
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-white font-semibold">Wealth over time</p>
            </div>
            <div className="flex items-center gap-2">
              <div className="live-dot" />
              <span className="text-xs font-mono" style={{ color: 'rgba(168,85,247,0.6)' }}>TRACKING</span>
            </div>
          </div>
      ...
      <div className="card p-4 md:p-5 mb-4" style={{ animationDelay: '320ms' }}>
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowSpendingHistory(true)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="rounded-xl p-4" style={{ background: 'rgba(168,85,247,0.05)', border: '1px solid rgba(168,85,247,0.14)' }}>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setShowWeekDetails(!showWeekDetails)}
                className="text-xs flex items-center gap-1" style={{ color: '#64748b' }}>
            ...
            {weekCats.length > 0 ? (
              <div className="space-y-2.5">
                {weekCats.map((c) => <CatBar key={c.key} label={c.label} value={c.value} total={weekTotal} color={c.color} />)}
              </div>
            ) : (
          ...
          <div className="rounded-xl p-4" style={{ background: 'rgba(168,85,247,0.05)', border: '1px solid rgba(168,85,247,0.14)' }}>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setShowMonthDetails(!showMonthDetails)}
                className="text-xs flex items-center gap-1" style={{ color: '#64748b' }}>
            ...
            {monthCats.length > 0 ? (
              <div className="space-y-2.5 mb-3">
                {monthCats.map((c) => <CatBar key={c.key} label={c.label} value={c.value} total={monthTotal} color={c.color} />)}
              </div>
            ) : (
          ...
          <div className="space-y-2.5">
            ...
            <div>
              ...
              <p className="text-xs font-mono mt-1" style={{ color: '#64748b' }}>
                = {fmt((parseFloat(draftGoldGrams) || data.goldGrams) * data.goldPricePerGram)} EGP
              </p>
            </div>
            <div>
              ...
              <p className="text-xs font-mono mt-1" style={{ color: '#64748b' }}>
                = {fmt((parseFloat(draftUsdAmt) || data.usdAmount) * data.usdToEGP)} EGP
              </p>
            </div>
          </div>
        </Modal>
            ...
            <div className="space-y-2 max-h-[60vh] overflow-y-auto">
              {[...data.spendingEntries].reverse().map((e) => (
                <div key={e.id} className="flex items-center justify-between rounded-xl px-3 py-2.5"
                  ...
                  <button onClick={() => deleteSpending(e.id)}
                    className="ml-3 opacity-30 hover:opacity-100 transition-opacity flex-shrink-0" style={{ color: '#f87171' }}>
                    <Trash2 size={14} />
                  </button>
                </div>
              ))}
      ...
      {showAddAsset && (
        <Modal
          title={showAddAsset === 'cash' ? 'Add Cash' : showAddAsset === 'gold' ? 'Add Gold' : 'Add USD'}
